...
{$ENDIF}
// je pøenesená daòová povinnost ?  19.10.2016
    Close;
...
// vyhledání údajù o smlouvách
    Close;
{$IFDEF ABAK}
    SQLStr := 'SELECT VS, AbraKod, Typ, Smlouva, AktivniOd, AktivniDo, FakturovatOd, Tarif, Posilani, Perioda, Text, Cena, DPH, Tarifni'
{$ELSE}
...
{$ENDIF}
    ...
    SQL.Text := SQLStr;
    Open;
    if Check then dmCommon.Zprava('Vyhledána data smlouvy v MySQL');
...
// 24.1.2017
    if FieldByName('CTU').AsString = '' then begin
...
{$IFDEF ABAK}
    if rbVoIP.Checked then begin
      FData.ValueByName('DocQueue_ID') := VDocQueue_Id;
      Description := 'VoIP, ';
    end;
{$ENDIF}
...
{$IFDEF ABAK}
    FData.ValueByName('PricesWithVAT') := False;
    FData.ValueByName('TotalRounding') := 257;                // zaokrouhlení na koruny
{$ELSE}
...
{$IFDEF ABAK}
    if FieldByName('Perioda').AsInteger = 3 then
      FRow.ValueByName('Text') := Format('Fakturujeme Vám za období od 1.%d.%d do %d.%d.%d',
       [aseMesic.Value, aseRok.Value, DayOfTheMonth(EndOfAMonth(aseRok.Value, aseMesic.Value+2)), aseMesic.Value+2, aseRok.Value]);
    if FieldByName('Perioda').AsInteger = 6 then
      FRow.ValueByName('Text') := Format('Fakturujeme Vám za období od 1.%d.%d do %d.%d.%d',
       [aseMesic.Value, aseRok.Value, DayOfTheMonth(EndOfAMonth(aseRok.Value, aseMesic.Value+5)), aseMesic.Value+5, aseRok.Value]);
    if FieldByName('Perioda').AsInteger = 12 then
      FRow.ValueByName('Text') := Format('Fakturujeme Vám za období od 1.%d.%d do %d.%d.%d',
       [aseMesic.Value, aseRok.Value, DayOfTheMonth(EndOfAMonth(aseRok.Value, aseMesic.Value+11)), aseMesic.Value+11, aseRok.Value]);
{$ENDIF}
    FRowsCollection.Add(FRow);
// další øádky faktury se vytvoøí z qrSmlouvy
    while not EOF do begin  // qrSmlouva
      CenaTarifu := 0;
      PausalVoIP := 0;
      HovorneVoIP := 0;
{$IFDEF ABAK}
// oddìlí se internetové a VoIPové smlouvy pod jedním VS
      if rbInternet.Checked and (FieldByName('Typ').AsString <> 'InternetContract') and (FieldByName('Typ').AsString <> 'StoryContract')
// 8.7.2016 or rbVoIP.Checked and (FieldByName('Typ').AsString <> 'VoipContract') or (FieldByName('Cena').AsFloat = 0) then begin
       or rbVoIP.Checked and (FieldByName('Typ').AsString <> 'VoipContract') then begin
        Next;
        Continue;
      end;
// jen pro nenulové ceny (u pøíbìhù ?) 8.7.2016 pøidán VoipContract
      if (FieldByName('Cena').AsFloat = 0)
       and (FieldByName('Typ').AsString <> 'StoryContract') and (FieldByName('Typ').AsString <> 'VoipContract') then begin
//        if (FieldByName('Cena').AsFloat = 0) and (FieldByName('Typ').AsString <> 'StoryContract') then begin
        Next;
        Continue;
      end;
// øádek s tarifem
      if (FieldByName('Tarifni').AsInteger = 1) then begin
        CenaTarifu := FieldByName('Cena').AsFloat * FieldByName('Perioda').AsInteger;
        FRow:= AbraOLE.CreateValues('@IssuedInvoiceRow');
        FRow.ValueByName('Division_ID') := '1000000101';
        if (FieldByName('Typ').AsString <> 'StoryContract') then FRow.ValueByName('Text') :=
          Format('podle smlouvy  %s  službu  %s %s', [FieldByName('Smlouva').AsString, FieldByName('Text').AsString, FieldByName('Cena').AsString])
        else FRow.ValueByName('Text') :=
          Format('podle smlouvy  %s  službu  %s', [FieldByName('Smlouva').AsString, FieldByName('Text').AsString]);
        if CenaTarifu > 0 then begin                     // tarify s mìsíèním paušálem
          ...
          FRow.ValueByName('IncomeType_ID') := '2000000101';
          FRow.ValueByName('TotalPrice') := Format('%f', [CenaTarifu]);
        end else FRow.ValueByName('RowType') := '0';       // a bez paušálu
        FRowsCollection.Add(FRow);
// VoIP - paušál a hovorné
        if rbVoIP.Checked then begin
          ProvolatelnyPausal := Pos('TP-', FieldByName('Tarif').AsString) > 0;
          with qrVoIP do begin
            Close;
            ...
            SQL.Text := SQLStr;
            Open;
            HovorneVoIP := Fields[0].AsFloat;
            Close;
            SQLStr := 'SELECT Price_Monthly FROM VoIP.Tariffs'
            + ' JOIN VoIP.Contracts ON Contracts.Tariff_Id = Tariffs.Id'
            + ' WHERE Contracts.Num = ' + qrSmlouva.FieldByName('Smlouva').AsString;
            SQL.Text := SQLStr;
            Open;
            PausalVoIP := Fields[0].AsFloat;
            Close;
            if Check then dmCommon.Zprava('Vyhledána data SIP');
          end;
          if ProvolatelnyPausal then HovorneVoIP := Max(0, HovorneVoIP - CenaTarifu);
        ...
        end;  // if rbVoIP.Checked
// nìco jiného než tarif
      end else begin
        FRow:= AbraOLE.CreateValues('@IssuedInvoiceRow');
        FRow.ValueByName('Division_ID') := '1000000101';
        if FieldByName('DPH').AsString = '21%' then begin
          FRow.ValueByName('VATRate_ID') := VATRate_Id;
          if DRC then begin                                              // 19.10.2016
            FRow.ValueByName('VATIndex_ID') := DRCVATIndex_Id;
            FRow.ValueByName('DRCArticle_ID') := DRCArticle_Id;          // typ plnìní 21
        ...
        FRow.ValueByName('TotalPrice') := Format('%f', [FieldByName('Cena').AsFloat * FieldByName('Perioda').AsInteger]);
        FRowsCollection.Add(FRow);
      end; // if tarif else
{$ELSE}
...
// 24.1.2017 zakázky pro ÈTÚ
      with qrAbra do begin
        Close;
        SQLStr := 'SELECT Id FROM BusOrders'
        + ' WHERE Code = ' + Ap + qrSmlouva.FieldByName('CTU').AsString + Ap;
        SQL.Text := SQLStr;
        Open;
...
{$IFDEF ABAK}
      if rbVoIP.Checked and (FCena = 0) then begin
        FObject.Delete(ID);
        dmCommon.Zprava(Format('%s (%s): Faktura %s mìla nulovou celkovou cenu. Smazána.', [Zakaznik, qrSmlouva.FieldByName('Smlouva').AsString, FStr]));
      end;
{$ENDIF}
    except
