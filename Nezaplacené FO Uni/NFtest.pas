...
{$IFDEF ABAK}
        Body.Text := StringReplace(mmMail.Text, '%%%', IntToStr(Round(Floats[3, Radek])), [rfIgnoreCase]);
        Body.Add(' ');
...
begin
    idHTTP.Request.Clear;
    idHTTP.Request.BasicAuthentication := True;
    idHTTP.Request.Username := 'jirka';
    idHTTP.Request.Password := 'heslo';
    URL := 'http://aplikace.eurosignal.cz';
        HTTPMessage := idHTTP.Get(Format('%s/api/sms/send?recipient=603246343&body=test', [URL]));
        if HTTPMessage <> 'OK' then
          if Application.MessageBox(PChar(Format('Omezení se nepodaøilo uložit do databáze: %s. Pokraèovat?', [HTTPMessage])),
           'Pozor', MB_ICONQUESTION + MB_YESNO + MB_DEFBUTTON1) = IDNO then Exit;
end;
